...
def get_kitti_dataset(data_dir):
	if not os.path.exists(data_dir):
		os.makedirs(data_dir)
		
	print('Downloading kitti data...')
	with DLProgress(unit='B', unit_scale=True, miniters=1) as pbar:
		urlretrieve('https://s3-us-west-1.amazonaws.com/udacity-selfdrivingcar/advanced_deep_learning/data_road.zip',
				os.path.join(data_dir, 'data_road.zip'),
				pbar.hook)
	...
	os.remove(os.path.join(data_dir, 'data_road.zip'))
	...
	def get_batches_fn(batch_size):
		...
		for batch_i in range(0, len(image_paths), batch_size):
			...
			for image_file in image_paths[batch_i:batch_i+batch_size]:
				...
				if augment:
					image, gt_image = randomize(image, gt_image)
